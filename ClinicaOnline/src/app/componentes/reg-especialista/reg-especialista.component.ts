...
@Component({
  ...
  styleUrls: ['./reg-especialista.component.scss']
})
export class RegEspecialistaComponent {
      ...
      data.forEach(esp => {
        if(esp.tipo === 'especialista')
        {
          ...
          esp.especialidad.forEach((e : any) => {
            let esp = JSON.parse(e);

            if(!this.especialidades.includes(esp.especialidad))
            {
              this.especialidades.push(esp.especialidad);
            }
          });
  ...
  alta()
  {
    if(!this.form.valid)
    {
      ...
      this.mensaje = 'Usted no completo todos los campos';
      ...
      this.guardarFoto('especialista', 'file1').then((data)=>{
        data.subscribe((url)=>{
          ...
          let especialista = {'nombre' : this.nombre, 
                            ...
                            'edad' : this.edad, 
                            'dni' : this.dni, 
                            'especialidad' : this.guardarEspecialidades(this.especialidad.split(' ')), 
                            'correo' : this.correo, 
                            'foto' : this.foto1,
                            'aprobado' : false,
                            'tipo' : 'especialista'};
  ...
  guardarEspecialidades(especialidades : any) : any[]
  {
    let especialidadesJSON : any [] = [];

    especialidades.forEach((e : any) => {
      if(e != '')
      {
        especialidadesJSON.push(`{"especialidad":"${e}", "duracionTurno":0, "dias":[]}`);
      }
    });
